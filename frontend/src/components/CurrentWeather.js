...
const CurrentWeather = () => {
  ...
  const [isoAlpha2, setIsoAlpha2] = useState(null);


  useEffect(() => {
    const fetchWeather = async () => {
      try {
        const { city: { name }, weather, flagUrl, isoAlpha2 } = await getWeather();
        console.log(name);
        setCity({ name }); // Set the random City name, flag, ISO Alpha data*/
        setWeather(weather);
        setFlagUrl(flagUrl);
  ...
  if (!weather || !city || !flagUrl || !isoAlpha2) return <p>Loading...</p>;
...
<div className="current-weather">
      <div className="current-weather-card">
        ...
        <div className="city-info">
          <img src={flagUrl} alt={`${isoAlpha2} flag`} className="flag-icon" />
          <span>{city?.name}, {isoAlpha2}</span>
        </div>
        <img 
          src={iconUrl} 
